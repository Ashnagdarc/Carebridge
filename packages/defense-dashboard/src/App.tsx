...
const wsUrl = import.meta.env.VITE_DEFENSE_WS_URL || 'ws://localhost:3000/ws/defense';
...
const redactPayload = (payload: DefenseEvent['payload']) => {
  ...
  Object.entries(payload).forEach(([key, value]) => {
    ...
    safe[key] =
      lower.includes('name') || lower.includes('email') || lower.includes('phone') || lower.includes('address')
        ? '[REDACTED]'
        : value;
  });
  return safe;
...
export default function App() {
  ...
  const requestEvents = useMemo(() => {
    const inferredRequestId = events.find((event) => event.payload.dataRequestId)?.payload.dataRequestId;
    const requestId = selectedRequestId || (processLocked ? null : inferredRequestId);
    if (!requestId) return [];
    return events
  ...
  const activeRequestId = requestEvents[0]?.payload.dataRequestId || selectedRequestId;
  ...
  const failed = requestEvents.some(
    (event) => event.type === 'consent_denied' || event.type === 'data_request_failed',
  );
  ...
  const completedSuccessfully = requestEvents.some((event) => event.type === 'data_request_completed');
  const waitingForApproval = hasConsentRequested && !hasConsentApproved && !hasFinalState;
  ...
  const consentRequestedAtMs = useMemo(() => {
    const consentEvent = requestEvents.find((event) => event.type === 'consent_request_created');
  ...
  const lookupPatient = async () => {
    ...
    try {
      ...
      setStartError(error instanceof Error ? error.message : 'Could not resolve patient from ID/QR value');
    } finally {
  ...
  const startDemo = async () => {
    ...
    try {
      if (hasActiveRun) {
        setStartError('Current request is still running. Wait until it finishes before starting a new one.');
        return;
      }
      ...
      if (!matchedPatient) {
        setStartError('Patient could not be verified from the provided ID/QR value.');
        return;
      }
      ...
      const response = await fetch(`${apiBase}/defense/start?token=${encodeURIComponent(wsToken)}`, {
        method: 'POST',
      ...
      setStartError(error instanceof Error ? error.message : 'Could not start demo flow');
    } finally {
      setStarting(false);
    }
  };
  ...
  const approveConsent = async () => {
    ...
    try {
      ...
      setStartError(error instanceof Error ? error.message : 'Could not approve consent');
    } finally {
  ...
  return (
    <PixelBackground className="page-wrap" gap={6} speed={35} pattern="center" darkColors="#173327,#25483a,#385f50">
      <main className="page">
        ...
        <header className="top">
          <div>
            ...
            <p>Realtime hospital-to-hospital flow with patient consent enforcement.</p>
          </div>
        ...
        <section className="control-bar">
          ...
          <div className="field">
            ...
            <select
              ...
              disabled={!lookupPerformed || waitingForApproval || waitingForRecords}
            >
              <option value="">{lookupPerformed ? 'Select source hospital' : 'Lookup patient first'}</option>
          ...
          <button
            className="start"
            onClick={startDemo}
            disabled={!canInitiateRecords || starting || waitingForApproval || waitingForRecords}
          >
        ...
        <section className="journey">
          <article className={`journey-step ${matchedPatient ? 'done' : lookupPerformed ? 'active' : ''}`}>
        ...
        {lookupPerformed && resolution && (
          <section className="patient-preview">
            {resolution.found && resolution.patient ? (
              <>
                <p>
                  <strong>Patient:</strong> {resolution.patient.fullName} ({resolution.patient.externalId})
                </p>
                <p>
                  <strong>Patient ID:</strong> {resolution.patient.id} | <strong>Email:</strong>{' '}
                  {resolution.patient.email}
                </p>
                <p>
                  <strong>Data likely available at:</strong>{' '}
                  {resolution.hospitals.length
                    ? resolution.hospitals.map((hospital) => `${hospital.name} (${hospital.code})`).join(', ')
        ...
        <section className="flow">
          <div className="flow-head">
            ...
            <div className="flow-head-actions">
              ...
              {processLocked && hasFinalState && (
                <button className="clear-process" onClick={clearCompletedProcess}>
                  Start New Process
                </button>
              )}
            </div>
          ...
          <div className={`track ${failed ? 'failed' : ''}`}>
            <span className={`beam ${stage > 0 ? 'active' : ''}`} style={{ ['--stage' as string]: String(stage) }} />
            {stageLabels.map((label, index) => {
              const step = index + 1;
              const on = stage >= step;
              return (
                <div key={label} className={`step ${on ? 'on' : ''} ${failed ? 'failed' : ''}`}>
                  <span className="node" />
                  <small>{label}</small>
                </div>
              );
          ...
          {waitingForRecords && <p className="process-lock-note">Consent approved. Retrieving records now...</p>}
        </section>
        ...
        <section className="content">
          <article className="panel">
            ...
            <div className="timeline">
              {events.length === 0 && <p className="empty">Waiting for realtime events...</p>}
              {events.map((event, index) => {
                const requestId = event.payload.dataRequestId || event.payload.consentRequestId || `event-${index}`;
                return (
                  <button
                    key={`${event.timestamp}-${requestId}-${index}`}
                    className={`row ${event.payload.dataRequestId === activeRequestId ? 'selected' : ''}`}
                    onClick={() => {
          ...
          <article className="panel">
            ...
            {!requestEvents.length && <p className="empty">Run a flow or select an event.</p>}

            {requestEvents.length > 0 && !resultsUnlocked && !failed && (
              <p className="empty">Records are locked until consent is approved and delivery is complete.</p>
            )}

            {failed && <p className="empty">Request failed before records could be delivered.</p>}
            ...
            {resultsUnlocked &&
              requestEvents.map((event, index) => (
                <div key={`${event.timestamp}-${index}`} className="event-card">
                  ...
                  <pre>{JSON.stringify(showRawPayload ? event.payload : redactPayload(event.payload), null, 2)}</pre>
                </div>
              ))}
          </article>
        </section>
        ...
        {waitingForApproval && (
          <section className="approval-overlay" aria-live="polite">
            <div className="approval-card">
              ...
              <div className="approval-wait-time">Waiting {formatElapsed(approvalWaitSeconds)}</div>
              <p>
                Initial records request is paused until patient consent is approved for request{' '}
                <strong>{activeRequestId}</strong>.
              </p>
              <button onClick={approveConsent} disabled={!canApproveConsent}>
